...
(function() {
  var _op, _val, all_types, augment, augment_and_filter_request, build_file_view, cmp, delete_button, die, doValidityCheck, do_request, do_slack_request, fields, filterPost, filterPre, generate_request, generate_where_row, generic_valid_text, getID, global_files, hash_filter, op_all, op_bool, op_cmp, op_no_regex, op_nocmp, operator_names, params, redir_submit_link, selectrowsEventHandler, time_to_unix_epoch, toBool, toBytes;
  ...
  time_to_unix_epoch = function(str) {
  ...
  do_request = function(params, callback, submit, users) {
    ...
    for (j = 0, len = users.length; j < len; j++) {
      ...
      if (_.contains(['>', '_'], _op(params.time))) {
        submit.ts_from = time_to_unix_epoch(_val(params.time));
      }
      if (_.contains(['<', '_'], _op(params.time))) {
        submit.ts_to = time_to_unix_epoch(_val(params.time));
      }
      if (user) {
        submit.user = user;
      }
  ...
  filterPre = function(files) {
    return _(files).filter(function(file) {
      var keep;
      keep = true;
      if (params.size) {
        if (!cmp(file.size, _op(params.size), toBytes(_val(params.size)))) {
          keep = false;
        }
      }
      if (params.name) {
        if (!cmp(file.name, params.name)) {
          keep = false;
        }
      }
      if (params.title) {
        if (!cmp(file.title, params.title)) {
          keep = false;
        }
      }
      if (params.is_public) {
        if (!cmp(file.is_public, _op(params.is_public), toBool(params.is_public))) {
          keep = false;
        }
      }
      return keep;
  ...
  filterPost = function(files) {
    return _(files).filter(function(file) {
      ...
      if (params.shared_in) {
        ...
        console.log("b: " + b);
        if (op === '~') {
          if (!_.any(a, function(chan) {
            return new RegExp(b).test(chan);
          })) {
            keep = false;
          }
        } else if (op === '_') {
