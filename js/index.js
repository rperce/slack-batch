...
(function() {
  var _op, _val, all_types, augment, augment_and_filter_request, build_file_view, delete_button, die, doValidityCheck, do_request, do_slack_request, fields, filter, generate_request, generate_where_row, generic_valid_text, getID, global_files, hash_filter, op_all, op_bool, op_cmp, op_no_regex, op_nocmp, operator_names, params, redir_submit_link, selectrowsEventHandler;
  ...
  window.onload = function() {
    ...
    generate_request(params, function(files) {
      return augment_and_filter_request(files, function() {
  ...
  doValidityCheck = function(event) {
    var field, html, input, mid, sel, style;
    input = $(event.target);
    field = input.parent();
    ...
    style = function(border, midHTML, valid_text) {
    ...
    if (input.val().length === 0 || !_(_(fields).keys()).contains(input.val())) {
      return style((input.val().length > 0 ? 'red' : '#aaa'), _.map(operator_names, function(v, k) {
      ...
      html = _.map(hash_filter(operator_names, function(v, k) {
        return k === '' || _.contains(fields[input.val()].ops, k);
      }), function(v, k) {
        return "<option value='" + k + "' " + (sel(mid.val(), k)) + ">" + v + "</option>";
      }).join("\n");
      return style('#aaa', html, fields[input.val()].valid_text);
    }
  };
